...
│   └── version.js          # 全局版本号 APP_VERSION，供 JS 与 fetch 缓存破坏使用
├── scripts/
│   └── bump-version.py     # 一键同步版本号到各文件
├── src/
│   └── py/
...
项目使用单点版本号 `version.json` 作为事实来源，当前版本为 `v1.1.2`。所有静态资源（CSS、JS、JSON 数据）在 HTML 与 `fetch` 中均附带 `?v=1.1.2` 缓存破坏参数，`ca.html` 右上角也会显示当前版本号。
...
- `version.json`
- `js/version.js` 中的 `APP_VERSION`
- `index.html` / `ca.html` 中所有 CSS/JS 链接的 `?v=` 参数
- `ca.html` 页面右上角的版本标签

部署时建议对 `index.html` 与 `ca.html` 设置较短的缓存时间（或 `Cache-Control: no-cache`），以确保用户能拿到带新版本戳的 HTML。
...
- **`css/GlobalStyle.css`**：提取了公共渐变类 `.ca-gradient-surface`、悬浮球组件、可拖拽悬浮窗组件以及 `#SidebarButtonArea` 等跨页面复用样式。
- **`js/index.js`**：
...
- **`js/CourseArrangement.js`**：
  - 通过 `fetch` 加载 `COURSE_DATA_PATH` 指向的 JSON，并附带 `APP_VERSION` 作为缓存破坏参数；
  - 提供课程、教师、时间三类输入建议；
